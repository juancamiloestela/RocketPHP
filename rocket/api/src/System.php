...
class System
{
	...
	protected $runtimeRoutes = array();
	protected $runtimeContexts = array();
	...
	public function launch(){//$db, $config = array()){
		//static::$config = array_merge(static::$defaults, $config);

		$this->runtimeContexts = include $this->config['core_path'] . 'contexts.php';
		$this->runtimeRoutes = include $this->config['core_path'] . 'routes.php';

		//include $this->config['contexts_file'];
		//include $this->config['delegates_file'];
...
//echo $method;
	...
	public function generateContexts()
	{
		...
		file_put_contents($this->config['core_path'] . 'contexts.php', '<?php ' . PHP_EOL . $src);
	}
	...
	public function generateResources()
	{
		...
		foreach ($this->specs->resources as $resourceName => $resource){
					...
					if ($property->type->relation == 'has-many'){
						if ($targetProperty->type->relation == 'has-many'){
							...
							if (isset($property->type->sql)){
								...
								echo "\t\t" . "\$query = \"SELECT * FROM {$property->type->resource} WHERE {$propertyName}_id = :id\";" . PHP_EOL;
							}
							echo "\t\t" . "\$statement = \$this->db->prepare(\$query);" . PHP_EOL;
							echo "\t\t" . "\$statement->execute(array('id' => \$id));" . PHP_EOL;
							echo "\t\t" . "\$data = \$statement->fetchAll(PDO::FETCH_ASSOC);" . PHP_EOL;
			...
			if (!file_exists($this->config['core_path'] . 'resources/')){
				mkdir($this->config['core_path'] . 'resources/', 0755, true);
			}
			file_put_contents($this->config['core_path'] . 'resources' . DIRECTORY_SEPARATOR . $resourceName . '.php', '<?php ' . PHP_EOL . $src);
		}
	}
	...
	public function generateRoutes()
	{
		...
		file_put_contents($this->config['core_path'] . 'routes.php', $routes);
	}
	...
	public function syncDb()
	{
		...
		$existingTables = $this->db->tables(true);
		$models = array();
...
//echo '<pre>';print_r($m);echo '</pre>';
		...
		foreach ($existingTables as $tableName){
			if (in_array($tableName, $this->config['ignored_tables'])){
				continue;
			}
			$dbSchema = $this->db->schema($tableName);
